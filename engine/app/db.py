...
def _run_migrations() -> None:
    """Run numbered SQL migration scripts from app/migrations/ directory."""
...
def add_audit_log(
    action: str, entity_type: str = "", entity_id: int = 0,
    channel_name: str = "", details: str = "",
    provider: str = "", model: str = "", tokens_used: int = 0, duration_ms: int = 0,
) -> int:
    now = utc_now_iso()
    with get_conn() as conn:
        try:
            cur = conn.execute(
                "INSERT INTO audit_logs(action, entity_type, entity_id, channel_name, details, provider, model, tokens_used, duration_ms, created_at) VALUES (?,?,?,?,?,?,?,?,?,?)",
                (action, entity_type, entity_id, channel_name, details[:2000], provider, model, tokens_used, duration_ms, now),
            )
            return int(cur.lastrowid)
        except Exception:
            return 0
...
def list_audit_logs(limit: int = 100, action: str | None = None, channel: str | None = None) -> list[dict[str, Any]]:
